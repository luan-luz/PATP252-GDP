...
public class CadastroCategoriaController {
    ...
    private List<Categoria> lstCadastrados;
    ...
    void cadastrarAPI(ActionEvent event) {
        try {
            if (!edtNome.getText().isBlank()) {
                ...
                map.put(1, new Categoria(edtNome.getText()));
                String body = mapper.writeValueAsString(map);
                RespostaHTTP resp = http.post("http://localhost:8080/categoria", body);
                if (resp.getHttpStatus() < 206) {
                    map.clear();
                    map = mapper.readValue(resp.getBody(), new TypeReference<Map<Integer, Categoria>>() {});
                    this.lstCadastrados = map.values().stream().toList();
                    showMessage(Alert.AlertType.INFORMATION,"Categoria cadastrada com sucesso!");
            ...
            showMessage(Alert.AlertType.ERROR, "Erro ao cadastrar Categoria: " + e.getMessage());
        }
    }
    public List<Categoria> getLstCadastrados() {
